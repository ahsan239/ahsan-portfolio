...
import { Github, Code2, Menu, X, User, LayoutGrid, Briefcase } from "lucide-react";
    ...
    >
      <div className="glass-card rounded-full px-6 h-14 flex items-center justify-between border-white/10 shadow-2xl bg-background/60 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="https://github.com/ahsan239" target="_blank" className="hidden md:flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground hover:text-foreground hover:bg-white/5 transition-all">
            <Github size={18} />
          </Link>
        </div>
      </div>
        ...
        <div className="absolute top-16 left-4 right-4 glass-card rounded-3xl p-6 border border-white/10 md:hidden animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="flex flex-col gap-4">
            ...
            <div className="pt-4 border-t border-white/10 flex items-center justify-between">
              <Link href="https://github.com/ahsan239" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary">
